...
function createWindow() {
  ...
  // Variável para armazenar o conteúdo da área de transferência
  ...
  setInterval(() => {
    ...
    if (currentClipboardContent !== lastClipboardContent) {
      console.log('Novo conteúdo copiado:', currentClipboardContent)
...
// Lidar com mensagens do renderer process
ipcMain.on('canal-exemplo', (event, args) => {
  console.log('Mensagem do frontend:', args)
  event.reply('resposta-exemplo', { success: true, message: 'Recebido!' })
})

ipcMain.handle('get-data', async (event, args) => {
  console.log('Parâmetros recebidos:', args)
  return { dados: 'Aqui está a resposta do backend!' }
})
